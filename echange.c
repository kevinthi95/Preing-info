...
int rechercheSym(int nb,int n,int m, char tab[n][m]){ // on teste l'alignement
  ...
  for( i=0; i<=n; i++){
    for(j=0; j<m; j++){
  if ((i>=2 && tab[i][j]==tab[i-1][j]) && (i>2 && tab[i-1][j]==tab[i-2][j])){ // colonne
    res=1;
  } 
  if((j<m && tab[i][j]==tab[i][j+1]) && (j<m-1 && tab[i][j+1]==tab[i][j+2])){ // ligne
    res= 6;
  }
  if((i>=2 && j<=m-1 && tab[i][j]==tab[i-1][j+1]) && (i>2 && j<m-1 &&  tab[i][j]==tab[i-2][j+2])){ // diagonale vers le haut
    res= 9;
  } 
    if((j<m && i<n && tab[i][j]==tab[i+1][j+1]) && (i<n-1 && j<m-1 && tab[i][j]==tab[i+2][j+2])){ // diagonale vers le bas 
  res= 10;
}
    }
  }
